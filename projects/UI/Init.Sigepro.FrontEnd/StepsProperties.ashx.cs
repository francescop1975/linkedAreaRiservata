    ...
    public class StepsProperties : IHttpHandler
    {
        ...
        public void ProcessRequest(HttpContext context)
        {
            ...
            foreach (var t in sortedList)
            {
                ...
                {
                    context.Response.Write($"<li>{p.Name} <b> [{p.PropertyType}]</b>");
                    var attribute = ((StepPropertyAttribute[])p.GetCustomAttributes(typeof(StepPropertyAttribute), true))?.FirstOrDefault();

                    if (attribute != null)
                    {
                        context.Response.Write("<br><i>");
                        context.Response.Write(attribute.Descrizione);

                        if (!String.IsNullOrEmpty(attribute.ValoreDefault))
                        {
                            context.Response.Write($" (Default: {attribute.ValoreDefault})");
                        }
