/**
 * questo modulo contiene una serie di funzioni per interrogare i layer del catasto
 */

/*** inizio importazioni dagli altri moduli ***/

//configurazione
import { config_ol } from "../../../config_modena_bootstrap.mjs";

//utilità per eseguire chiamate WFS/WMS a GEOSERVER
import { gisUtils } from "./gisUtils.mjs";
...
export var catastoUtils = {
     ...
     */
    requestAllFogli: function(successFunction, errorFunction) {
        ...
        var optionsObj = gisUtils.prepareWFSRequestParams(
            ...
            config_ol.gs_default_workspace_URI, [config_ol.layer_foglio.field_name_foglio],
     ...
     */
    requestAllParticelleByFoglio: function(foglio, successFunction, errorFunction) {
        if (commonUtils.string.isEmptyOrNull(foglio)) {
            ...
            var optionsObj = gisUtils.prepareWFSRequestParams(
                ...
                config_ol.gs_default_workspace_name,
                config_ol.gs_default_workspace_URI, [config_ol.layer_particella.field_name_foglio, config_ol.layer_particella.field_name_mappale, config_ol.layer_particella.field_name_partkey],
                config_ol.coord_system_code,
                false,
                filter,
                gisUtils.filter_type_cql,
     ...
     */
    requestSelectedParticelle: function(lista_partkey, successFunction, errorFunction) {
        ...
        if (lista_partkey != undefined && lista_partkey.length > 0) {
            ...
            var optionsObj = gisUtils.prepareWFSRequestParams(
                config_ol.layer_particella.layer_name,
                config_ol.gs_default_workspace_name,
                config_ol.gs_default_workspace_URI, [config_ol.layer_particella.field_name_geom, config_ol.layer_particella.field_name_foglio, config_ol.layer_particella.field_name_mappale, config_ol.layer_particella.field_name_partkey],
                config_ol.coord_system_code,
                false,
                filter,
                gisUtils.filter_type_cql,
     ...
     * @param {number} numeroFoglio - numero di foglio catastale
     ...
     */
    requestFoglioByNumero: function(numeroFoglio, successFunction, errorFunction) {
        ...
        var optionsObj = gisUtils.prepareWFSRequestParams(
            config_ol.layer_foglio.layer_name,
            config_ol.gs_default_workspace_name,
            config_ol.gs_default_workspace_URI, [config_ol.layer_foglio.field_name_geom, config_ol.layer_foglio.field_name_foglio],
            config_ol.coord_system_code,
            false,
            filter,
            gisUtils.filter_type_cql
     ...
     * @param {function} successFunction - funzione da eseguire in caso di esito positivo della richiesta WFS
     */
    requestFoglioByIntersectionPoint: function(pointX, pointY, successFunction) {
        ...
        var wfsSuccessCallBack =
            function(jsonResponse) {
                ...
                if (features != undefined && features[0] != undefined) {
                    catastoUtils.requestFoglioByNumero(
     ...
     */
    requestParticellaByIntersectionPoint: function(pointX, pointY, successFunction, errorFunction) {
        var filter = gisUtils.prepareIntersectWithPointFilter(false, config_ol.layer_particella.field_name_geom, pointX, pointY, config_ol.coord_system_code);
     ...
     */
    isAllParticelleOfSameFoglio: function(foglioPrincipale, arrayParticelle) {
        var foundDifferentFoglio = false;
        if (arrayParticelle != undefined) {
            for (var i = 0; i < arrayParticelle.length; i++) {
                var currentFoglio = catastoUtils.getNumeroFoglioFromPartkey(arrayParticelle[i]);
                if (currentFoglio != foglioPrincipale) {
     ...
     * i foglio e i suoi eventuali frazionamenti devono essere considerati tutti un solo foglio
     * 
     * @param {string} partkey 
     */
    getNumeroFoglioFromPartkey: function(partkey) {
     ...
     * @param {string} partkey 
     */
    getValoreMappaleFromPartkey: function(partkey) {
        if (partkey != undefined && partkey.length > 0) {
            //
            //esempio di PARTKEY: F257  257  100
            //                    _____||||=====  
            return partkey.substring(9).trim().toUpperCase();
        }
        return undefined;
    },
    ...
    //config_ol.layer_foglio.value_sezione
     ...
     * @param {*} mappale 
     */
    createPartkey: function(ente, foglio, mappale) {

        //aggiungo ENTE
        var partkey = ente.trim().padEnd(5, ' ');
        //aggiungo FOGLIO
        partkey = partkey.concat(foglio.trim().padStart(4, ' '));
        //aggiungo MAPPALE
        mappale = mappale.trim();
        if (isNaN(mappale)) {
            //mappale alfabetico
            partkey = partkey.concat(mappale.trim().padEnd(5, ' '));
        } else {
            //mappale numerico
            partkey = partkey.concat(mappale.trim().padStart(5, ' '));
        }
        return partkey;
    },
