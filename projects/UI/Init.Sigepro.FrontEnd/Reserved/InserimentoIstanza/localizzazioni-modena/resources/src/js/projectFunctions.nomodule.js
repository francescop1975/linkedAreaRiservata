/**
 * modulo contenente le funzioni di implementazione del programma
 */
...
function setArrayParticelleInput(newArray) {
						...
						if (features != undefined && features.length > 0) {
							...
							for (var i = 0; i < features.length; i++) {
								...
								if (currentPartkeyIndex > -1) {
									...
									//aggiungo paricella anche ad array navigation
 ...
 */
function eliminaPartKeyGraficoDaSelezione(partkey) {
 ...
 */
function eliminaPartKeyManualeDaSelezione(partkey) {
	//verifico se particella effettivamente selezionata
...
function onModalShow(e) {
	...
	modalParticellaValue.css("border-color", "black");
	setModalParticellaValidity("");
...
//	elem.setCustomValidity(msg);
	if (msg != "") {
    	modalParticellaValue.popover({
            placement:"right",
            trigger:"manual",
            html:true,
            content:msg
        });
		modalParticellaValue.popover("show");
		modalParticellaValue.css("outline-style", "solid");
		modalParticellaValue.css("outline-color", "red");
	} else {
		modalParticellaValue.popover("dispose");
		modalParticellaValue.css("outline-style", "none");
	}	
}