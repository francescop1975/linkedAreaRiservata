...
namespace Init.Sigepro.FrontEnd.Reserved.InserimentoIstanza.Pagamenti
{
    public partial class GestionePagamentiEntraNext : IstanzeStepPage
    {
        ...
        {
            OneriDomandaService.SincronizzaOneri(IdDomanda, ComportamentoSincronizzazioneOneriSenzaImporto.Ignora);
        }
        ...
        public override bool CanExitStep()
        {
            ...
            {
                Errori.Add("Tutti gli oneri della domanda devono essere pagati on-line");

                return false;
            }

            if (new NessunOnereDovutoSpecification().AndNot(new UtenteDichiaraDiNonAvereOneriSpecification()).IsSatisfiedBy(ReadFacade.Domanda.Oneri))
            {
                Errori.Add($"Gli oneri della domanda non sono dovuti. È necessario dichiarare che \"{this.TestoDichiarazioneAssenzaOneri}\"");
        ...
        protected List<KeyValuePair<string, string>> GetModalitaPagamentoSupportate()
        {
