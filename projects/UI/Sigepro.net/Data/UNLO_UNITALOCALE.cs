...
*/
		...
		public int Insert(string p_connectionString, DataBase p_db)
		{
			...
			dbCmd.CommandText = @"SELECT PV,N_REA," + db.Specifics.SubstrFunction("UL_SEDE", 4, 1) + " AS PROGUL,DENOMINAZIONE,to_date(DT_APER_UL,'dd/MM/yyyy'),STRAD,COMUNE,FRAZIONE,INDIRIZZO,CAP,to_date(DT_CESSAZ,'dd/MM/yyyy'),to_date(DT_CES_AT,'dd/MM/yyyy') FROM TMP_IMPORT WHERE UL_SEDE <> 'SEDE'";
