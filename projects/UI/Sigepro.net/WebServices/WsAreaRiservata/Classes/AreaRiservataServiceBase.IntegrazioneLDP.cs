...
namespace Sigepro.net.WebServices.WsAreaRiservata.Classes
{
	public partial class AreaRiservataServiceBase
	{
        ...
        [WebMethod]
        public ConfigurazioneAlberoprocLDP GetConfigurazioneAlberoprocLDPDaCodiceIstanza(string token, int codiceIstanza)
        {
            var auth = CheckToken(token);
            ...
            using (var db = auth.CreateDatabase())
            {
                ...
                return svc.GetConfigurazioneDaCodiceIstanza(codiceIstanza);
            }
