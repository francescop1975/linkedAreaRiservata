...
namespace Sigepro.net.WebServices.WsAreaRiservata
{
	...
	public class DatiConfigurazioneAreaRiservataService : System.Web.Services.WebService
	{
		...
		public ConfigurazioneAreaRiservataDto LeggiConfigurazione(string token, string software)
		{
			...
			{
                var softwareAttivi = new SoftwareAttiviMgr(db).GetSoftwareAttivi(authInfo.IdComune);
                ...
                var configurazioneGeneraleSoftware = new ConfigurazioneMgr(db).GetById(authInfo.IdComune, software);
                ...
                // Verticalizzazione Google Maps
                var vertGoogleMaps = new VerticalizzazioneGoogleMaps(authInfo.Alias, software);

                if (vertGoogleMaps.Attiva)
                {
                    rVal.GoogleMaps = new ParametriGoogleMaps
                    {
                        ApiKey = vertGoogleMaps.ApiKey,
                        MapBounds = vertGoogleMaps.MapBounds
                    };
                }

                // SIT attivo
                var verticalizzazioneSitAttivo = new VerticalizzazioneSitAttivo(authInfo.Alias, software);
