...
namespace SIGePro.Net.WebServices.WsSIGePro
{
    ...
    public class ProtocollazioneService : IProtocollazioneService
    {
        ...
        public DatiProtocolloRes ProtocollazioneComunicazioneGraduatoria(string token, string codiceMovimento)
        {
            ...
            {
                var fmtMsg = "ERRORE GENERATO DURANTE LA PROTOCOLLAZIONE DI UNA COMUNICAZIONE, {0}";
                var errore = String.Format(fmtMsg, ex.Message);
                _log.ErrorFormat(fmtMsg, ex.ToString());
                return new DatiProtocolloRes(errore, ex.ToString());
            }
        ...
        public DatiProtocolloLetto LeggiProtocollo(string token, string idProtocollo, string annoProtocollo, string numProtocollo, string software, string codiceComune)
        {
            ...
            catch (Exception ex)
            {
                ...
                try
                {
                    ...
                    return LeggiProtocolloConData(token, idProtocollo, dataProtocollo, numProtocollo, software, codiceComune);
                }
                catch (Exception ex2)
                {
                    ...
                    _log.ErrorFormat(fmtMsg, ex2.ToString());
                    return new DatiProtocolloLetto(ex2.Message, ex2.ToString());
                }
            }
        }
        ...
        /// <param name="codiceComune"></param>
        /// <returns></returns>
        public DatiProtocolloLetto LeggiProtocolloConData(string token, string idProtocollo, DateTime dataProtocollo, string numProtocollo, string software, string codiceComune)
        {
            _log.DebugFormat("Avvio lettura protocollo, metodo LeggiProtocolloConData, Token: {0}, Id Protocollo: {1}, Data Protocollo: {2}, Numero Protocollo: {3}, Software: {4}, Codice Comune: {5}", token, idProtocollo, dataProtocollo, numProtocollo, software, codiceComune);
        ...
        public AllOut LeggiAllegato(string token, string idBase, string software, string codiceComune)
        {
            ...
            try
            {
                ...
                using (var mgr = new ProtocolloMgr(authInfo, software, codiceComune))
                {
                    ...
                    var allegato = mgr.LeggiAllegato(idProtocollo, numProtocollo, annoProtocollo, idAllegato);
            ...
            catch (Exception ex)
            {
                ...
                try
                {
                    return LeggiAllegatoStorico(token, idBase, software, codiceComune);
                }
                catch (Exception ex2)
                {
                    ...
                    _log.ErrorFormat(fmtMsg, ex2.ToString());
                    return new AllOut(ex2.Message, ex2.ToString());
                }
            }
        }

        public AllOut LeggiAllegatoStorico(string token, string idBase, string software, string codiceComune)
        {
            _log.DebugFormat("Avvio lettura allegato, metodo LeggiAllegatoStorico, token: {0}, Id Base: {1}, Software: {2}, Codice Comune", token, idBase, software, codiceComune);
            ...
            try
            {
                ...
                using (var mgr = new ProtocolloMgr(authInfo, software, codiceComune))
                {
                    ...
                    return mgr.LeggiAllegatoStorico(idProtocollo, numProtocollo, annoProtocollo, idAllegato);
                }
            }
            catch (Exception ex)
            {
        ...
        private bool DocumentiIstanzaSenzaCodiceOggetto(AuthenticationInfo authInfo, Istanze istanza)
        {
            ...
            var vert = new VerticalizzazioneProtocolloAttivo(authInfo.Alias, software, codiceComune);
