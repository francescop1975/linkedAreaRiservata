...
public partial class Archivi_DatiDinamici_Dyn2Campi : BasePage
{
	...
	protected void cmdSalva_Click(object sender, EventArgs e)
	{
		...
		try
		{
            if (txtEtichetta.Value.Length > 2000)
            {
                throw new Exception("Il campo " + txtEtichetta.Descrizione + " non può contenere più di 2000 caratteri");
            }

            if (txtDescrizione.Value.Length > 4000)
            {
                throw new Exception("Il campo " + txtDescrizione.Descrizione + " non può contenere più di 2000 caratteri");
            }


            cls.Nomecampo = txtNomeCampo.Value.ToUpper();
            if (ddlTipoDato.Value != "TestoInSolaLettura")
            {
