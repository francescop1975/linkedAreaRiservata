...
namespace Init.Sigepro.FrontEnd.AppLogic.Adapters.StcPartialAdapters
{
	internal class AltriDatiAdapter : IStcPartialAdapter
	{
        ...
        IConfigurazione<ParametriGoogleMaps> _googleMapsConfig;
		...
		internal AltriDatiAdapter(ICodiceAccreditamentoHelper codiceAccreditamentoHelper)
		{
            ...
            this._googleMapsConfig = FoKernelContainer.GetService<IConfigurazione<ParametriGoogleMaps>>();
		...
		public void Adapt(GestionePresentazioneDomanda.IDomandaOnlineReadInterface _readInterface, StcService.DettaglioPraticaType _dettaglioPratica)
		{
            ...
            // Google maps
            if (_readInterface.DatiExtra.Keys.Contains(this._googleMapsConfig.Parametri.DatiExtraKey))
            {
                var altriDatiBase64 = _readInterface.DatiExtra.Get<string>(this._googleMapsConfig.Parametri.DatiExtraKey);
                var altriDatiJsonBytes = Convert.FromBase64String(altriDatiBase64);
                var altriDatiJsonString = Encoding.UTF8.GetString(altriDatiJsonBytes);
                
                listaAltriDati.Add(_parametriHelper.CreaParametroType(this._googleMapsConfig.Parametri.NomeTagAltriDati, altriDatiJsonString));
            }
