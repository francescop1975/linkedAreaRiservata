...
namespace Init.Sigepro.FrontEnd.AppLogic.GestionePresentazioneDomanda.GestioneDatiDinamici
{
	public class DatiDinamiciReadInterface: IDatiDinamiciReadInterface
	{
		...
		public IEnumerable<string> VerificaUploadModelliRichiesti(bool ignoraObbligoFirma)
		{
			...
			foreach (var r in rows)
			{
				...
				if (modello.TipoFirma != ModelloDinamico.TipoFirmaEnum.Nessuna)
					errori.Add($"Scaricare {(ignoraObbligoFirma? "" : ", firmare")} e allegare il file relativo alla dichiarazione/scheda \"" + r.Descrizione + "\"");
			}

			if (ignoraObbligoFirma)
            {
				return errori;
            }
