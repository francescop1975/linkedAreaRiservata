...
namespace Init.Sigepro.FrontEnd.AppLogic.GestionePresentazioneDomanda.GestioneAnagrafiche
{
    ...
    public class TutteLeTipologieDiSoggettoSonoCensiteEValide : ValidazioneAnagraficheDomandaBase
    {
        protected override void DoValidation(IAnagraficheReadInterface readInterface)
        {
            var listaRichiedenti = readInterface.GetRichiedenti();
            if (listaRichiedenti.Any(x => x.TipoSoggetto == null || !x.TipoSoggetto.Id.HasValue || x.TipoSoggetto.Id.Value <= 0))
                AddError("Ci sono dei soggetti che non hanno una qualifica valida per la domanda che si sta presentando.");
        }
	...
	public class EsisteAlmenoUnSoggettoRichiedente : ValidazioneAnagraficheDomandaBase
	{
        ...
        public EsisteAlmenoUnSoggettoRichiedente( bool richiedenteSoloPersoneFisiche)
        {
            this._RichiedenteSoloPersoneFisiche = richiedenteSoloPersoneFisiche;
        }
		...
		protected override void DoValidation(IAnagraficheReadInterface readInterface)
		{
            ...
            if( this._RichiedenteSoloPersoneFisiche && !listaRichiedenti.Where(x => x.TipoPersona == TipoPersonaEnum.Fisica).Any() )
            {
                AddError("Tra le anagrafiche non è stata individuata nessuna persona fisica con una tipologia di soggetto identificabile come richiedente");
            }
		}
	}
    ...
    /// <summary>
    /// Verifica che non siano immesse persone giuridiche senza essere collegate ad un legale rappresentante
    /// </summary>
    public class EsisteUnRappresentantePerTutteLePersoneGiuridicheIndicate : ValidazioneAnagraficheDomandaBase
    {
        ITipiSoggettoService _tipiSoggettoService;

        public EsisteUnRappresentantePerTutteLePersoneGiuridicheIndicate(ITipiSoggettoService tipiSoggettoService)
        {
            Condition.Requires(tipiSoggettoService, "tipiSoggettoService").IsNotNull();
        ...
        protected override void DoValidation(IAnagraficheReadInterface readInterface)
        {
            foreach (var anagrafica in readInterface.GetAnagraficheCollegabili())
            {
                if (anagrafica.TipoSoggetto.Ruolo != RuoloTipoSoggettoDomandaEnum.Richiedente &&  readInterface.GetLegaleRappresentanteDi(anagrafica, _tipiSoggettoService) == null)
                    AddError($"Non è stato indicato un rappresentante legale per l'azienda {anagrafica.Nominativo}");
            }
        }
    }
