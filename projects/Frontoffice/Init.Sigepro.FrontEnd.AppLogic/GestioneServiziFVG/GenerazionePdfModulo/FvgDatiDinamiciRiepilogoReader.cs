...
namespace Init.Sigepro.FrontEnd.AppLogic.GestioneServiziFVG.GenerazionePdfModulo
{
    public class FvgDatiDinamiciRiepilogoReader : IDatiDinamiciRiepilogoReader
    {
        ...
        public IDyn2DataAccessFactory CreateDataAccessProvider(int idScheda, ITokenApplicazioneService tokenApplicazioneService)
        {
            var cache = this._datiDinamiciRepository.GetCacheModelloDinamico(idScheda);

            return new FvgDyn2DataAccessFactory(cache, this._database, tokenApplicazioneService);
        }

        public IDyn2DataAccessFactory CreateDataAccessProviderStampaMolteplicita(int idScheda, int indiceMolteplicita, ITokenApplicazioneService tokenApplicazioneService)
        {
            return CreateDataAccessProvider(idScheda, tokenApplicazioneService);
        }

        public CampiNonVisibili GetCampiNonVisibili(int idModello)
        {
            var campiNonVisibili = this._database.GetCampiNonVisibili(idModello);
            return new CampiNonVisibili(campiNonVisibili);
        }
