...
namespace Init.Sigepro.FrontEnd.Pagamenti.ENTRANEXT
{
    public class PaymentRequestAdapterEntraNext
    {
        ...
        {
            public const int LunghezzaMassimaDescrizione = 35;
        }

        private readonly IUrlEncoder _urlEncoder = new HttpUtilityUrlEncoder();
        private readonly IResolveUrl _resolveUrl = new HttpContextResolveUrl();

        private readonly PaymentSettingsEntraNext _settings;
        private readonly IniziaPagamentoEntraNextRequest _iniziaPagamentoRequest;
        ...
        {
            var urlBack = new UrlPagamenti(this._settings.UrlBack, this._iniziaPagamentoRequest.RiferimentiDomanda, this._urlEncoder, this._resolveUrl).ToString() + "&esito=ANNULLATO";
            ...
            return new InserisciPosizioniInAttesaRequest
            {
                ...
                PosizioniDebitorie = new PosizioneDebitoriaInAttesa[]
                {
                    new PosizioneDebitoriaInAttesa
                    {
                        ...
                        Descrizione = (this._iniziaPagamentoRequest.RiferimentiDomanda.DescrizioneIntervento.Length > Constants.LunghezzaMassimaDescrizione ?
                                      this._iniziaPagamentoRequest.RiferimentiDomanda.DescrizioneIntervento.Substring(0,Constants.LunghezzaMassimaDescrizione):
                                      this._iniziaPagamentoRequest.RiferimentiDomanda.DescrizioneIntervento),
                        Annullato = false,
                        GestioneIva = false,
