...
namespace Init.Sigepro.FrontEnd.WebControls.Visura
{
	...
	public abstract class FiltriVisuraControlBase : System.Web.UI.WebControls.WebControl, IDatabaseSoftwareControl
	{
		...
		private void FillDictionary()
		{
			...
			//fabbricato
			_fabbricato.ID = "fabbricato";
			_fabbricato.IdRisorsa = "visura.fabbricato";
			_fabbricato.MaxLength = 10;
			_fabbricato.BtSize = BootstrapSize.Col3;

			// statoIstanza
			_statoIstanza.ID = "statoIstanza";
			_statoIstanza.IdRisorsa = "visura.stati_istanze";
			_statoIstanza.BtSize = BootstrapSize.Col3;

			// dataProtocollo
			_dataProtocollo.ID = "dataProtocollo";
			_dataProtocollo.IdRisorsa = "visura.data_protocollo";
			...
			_richiedente.IdRisorsa = "visura.richiedente";
			_richiedente.BtSize = BootstrapSize.Col3;

			// tipoIntervento
			_intervento.ID = "tipoIntervento";
			_intervento.IdRisorsa = "visura.tipo_intervento";
			_intervento.CssClass = "ricerca-intervento";
            _intervento.BtSize = BootstrapSize.Col4;

			// posizione archivio
			_posizioneArchivio.ID = "posizioneArchivio";
			_posizioneArchivio.MaxLength = 100;
			_posizioneArchivio.IdRisorsa = "visura.posizione_in_archivio";
			_posizioneArchivio.BtSize = BootstrapSize.Col3;
			...
			m_dictionary.Add(m_provider.IdFabbricato, _fabbricato);
			m_dictionary.Add(m_provider.IdPoszioneArchivio, _posizioneArchivio);
		}
		...
		private void CostruisciControlli()
		{
			...
			var filtri = m_provider.GetCampiFiltro(IdComune, Software).Where( x => !String.IsNullOrEmpty(x.Valore) && x.Valore != "0").OrderBy( x => x.Valore).ThenBy( x => x.Etichetta).ToArray();
