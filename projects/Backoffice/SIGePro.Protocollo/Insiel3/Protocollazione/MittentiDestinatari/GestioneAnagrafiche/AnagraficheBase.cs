...
namespace Init.SIGePro.Protocollo.Insiel3.Protocollazione.MittentiDestinatari.GestioneAnagrafiche
{
    public class AnagraficheBase
    {
        ...
        public AnagraficheBase(InsielVerticalizzazioniConfiguration vert)
        {
            this._vert = vert;
        }

        protected void AggiornaAnagrafica(IAnagraficaAmministrazione anagrafica, ProtocolloService srv, string nominativo, AnagraficaView responseLeggi, ProtocolloLogs logs)
        {
            ...
            logs.Info($"CONFIGURAZIONE AGGIORNAMENTO: {this._vert.TipoAggiornamentoAnagrafica}");
        ...
        protected void InserisciAnagraficaDefault(IAnagraficaAmministrazione anagrafica, ProtocolloService srv, string nominativo)
        {
            ...
            if (!this._vert.DisabilitaValidazioneCapIta && !String.IsNullOrEmpty(anagrafica.Cap))
            {
                ...
                if (anagrafica.Cap.Trim().Length < 4)
                {
                    throw new Exception($"IL VALORE {anagrafica.Cap.Trim()} RELATIVO AL CAP DI RESIDENZA DELL'ANAGRAFICA {anagrafica.NomeCognome} E' COMPOSTO DA MENO DI 4 CARATTERI");
                }
