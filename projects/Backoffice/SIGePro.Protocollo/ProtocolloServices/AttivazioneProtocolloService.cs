...
namespace Init.SIGePro.Protocollo.ProtocolloServices
{
    public class AttivazioneProtocolloService
    {
        ...
        public VerticalizzazioneProtocolloStorico VertProtocolloStorico { get; private set; }
        ...
        public AttivazioneProtocolloService(ResolveDatiProtocollazioneService datiProtocollazione, ProtocolloLogs protocolloLogs)
        {
            ...
            VertProtocolloStorico = new VerticalizzazioneProtocolloStorico(this._datiProtocollazione.IdComuneAlias, this._datiProtocollazione.Software, this._datiProtocollazione.CodiceComune);
        }
        ...
        public ProtocolloBase AttivaProtocolloStorico()
        {
            var tipoProtocollo = GetTipoProtocollo(this.VertProtocolloStorico);
            var objAssembly = Assembly.Load("SIGePro.Protocollo");

            if (tipoProtocollo == ProtocolloEnum.TipiProtocollo.NESSUNO)
                throw new Exception($"LA VERTICALIZZAZIONE {this.VertProtocolloStorico.Nome} NON E' VALORIZZATA");
        ...
        private ProtocolloEnum.TipiProtocollo GetTipoProtocollo(IVerticalizzazioneProtocollo verticalizzazione)
        {
            if (!verticalizzazione.Attiva)
                throw new Exception( $"LA VERTICALIZZAZIONE {verticalizzazione.Nome} NON E' ATTIVA");

            if( String.IsNullOrEmpty(verticalizzazione.Tipoprotocollo) )
                throw new Exception($"LA VERTICALIZZAZIONE {verticalizzazione.Nome} NON HA IL PARAMETRO TIPOPROTOCOLLO SPECIFICATO!");

            _protocolloLogs.DebugFormat("VERTICALIZZAZIONE {0} ATTIVA {0}", verticalizzazione.Nome, verticalizzazione.Attiva);

            return (ProtocolloEnum.TipiProtocollo)Enum.Parse(typeof(ProtocolloEnum.TipiProtocollo), verticalizzazione.Tipoprotocollo, false);
        }
    }
}
