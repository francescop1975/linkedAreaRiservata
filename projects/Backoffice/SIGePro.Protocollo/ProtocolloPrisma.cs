...
namespace Init.SIGePro.Protocollo
{
    public class PROTOCOLLO_PRISMA : ProtocolloBase
    {
        public override void InizializzaProtocolloBase(ResolveDatiProtocollazioneService datiProtocolloService)
        {
            ...
            base._protocolloSerializer = new PrismaSerializer(this._protocolloLogs);
        }
        ...
        public override DatiProtocolloLetto LeggiProtocollo(string idProtocollo, string annoProtocollo, string numeroProtocollo)
        {
            try
            {
                var serializer = new PrismaSerializer(this._protocolloLogs);

                this._protocolloLogs.DebugFormat($"Inizio lettura protocollo id: {idProtocollo}, anno: {annoProtocollo}, numero: {numeroProtocollo}");
                ...
                var authService = new AuthenticationServiceWrapper(vert.UrlProtoDocArea, this._protocolloLogs, serializer, new CredentialsInfo(vert.Username, vert.Password, vert.CodiceEnte, ""));
                var token = authService.Login();

                var service = new LeggiProtocolloServiceWrapper(vert.UrlExtended, this._protocolloLogs, serializer, new CredentialsInfo(vert.Username, vert.Password, vert.CodiceEnte, token));
                ...
                var serviceAllegati = new AllegatiServiceWrapper(vert.UrlAllegati, this._protocolloLogs, serializer, new CredentialsInfo(vert.Username, vert.Password, vert.CodiceEnte, token));

                var adapter = new LeggiProtocolloOutAdapter(serializer);

                this._protocolloLogs.DebugFormat($"Fine lettura protocollo id: {idProtocollo}, anno: {annoProtocollo}, numero: {numeroProtocollo}");

                return adapter.Adatta(response, responseDatiPec, serviceAllegati, this._protocolloLogs);
            }
            catch (Exception ex)
            {
