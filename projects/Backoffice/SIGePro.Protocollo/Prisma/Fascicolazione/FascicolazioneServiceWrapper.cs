...
namespace Init.SIGePro.Protocollo.Prisma.Fascicolazione
{
    public class FascicolazioneServiceWrapper : ExtendedServiceWrapper
    {
        ...
        public FascicolazioneServiceWrapper(string url, ProtocolloLogs logs, IProtocolloSerializer serializer, CredentialsInfo credentials) : base(url, logs, credentials)
        {
            ...
            try
            {
                using (var ws = base.CreaWebService())
                {
                    using (OperationContextScope scope = new OperationContextScope(ws.InnerChannel))
                    {
                        ...
                        var response = this._serializer.Deserialize<LeggiFascicoliOutXML>(responseXml);
        ...
        public CreaFascicoloOutXML CreaFascicolo(CreaFascicoloInXML request)
        {
            try
            {
                using (var ws = base.CreaWebService())
                {
                    ...
                    {
                        base.AggiungiCredenzialiAContextScope();
                        var requestXml = this._serializer.Serialize(ProtocolloLogsConstants.CreaFascicoloRequestFileName, request);
                        ...
                        var response = this._serializer.Deserialize<CreaFascicoloOutXML>(responseXml);
        ...
        public FascicolazioneOutXML FascicolaProtocollo(FascicolazioneInXML request)
        {
            try
            {
                using (var ws = base.CreaWebService())
                {
                    ...
                    {
                        base.AggiungiCredenzialiAContextScope();
                        var requestXml = this._serializer.Serialize(ProtocolloLogsConstants.FascicolazioneRequestFileName, request);
                        ...
                        var response = this._serializer.Deserialize<FascicolazioneOutXML>(responseXml);
        ...
        public void CambiaFascicolo(CambiaFascicoloInXML request)
        {
            try
            {
                using (var ws = base.CreaWebService())
                {
                    ...
                    {
                        base.AggiungiCredenzialiAContextScope();
                        var requestXml = this._serializer.Serialize(ProtocolloLogsConstants.CambiaFascicoloRequest, request);
                        ...
                        var response = this._serializer.Deserialize<CambiaFascicoloOutXML>(responseXml);
