...
namespace Init.SIGePro.Sit.Ravenna2
{
    public class Ravenna2DbClient
    {
        ...
        private readonly IDatabaseTableNameProvider _nameProvider;
        private readonly DataBase _db;
        private readonly ILog _log = LogManager.GetLogger(typeof(Ravenna2DbClient));

        string _codViarioNonDefinito = "0";

        public Ravenna2DbClient(DataBase db, string tablePrefix, IDatabaseTableNameProvider nameProvider)
        {
        ...
        internal Ravenna2ResultSet GetListaCivici(string codVia)
        {
            ...
            var ra012 = new Ra012Table(this._tablePrefix, this._nameProvider);
        ...
        internal DettagliVia[] GetListaVie()
        {
            ...
            var qTable = new Ra012Table(this._tablePrefix, this._nameProvider);
            var sql = $"select distinct substr(TOPONIMO, 0, instr(TOPONIMO, ' ')) as TOPONIMO, substr(TOPONIMO, instr(TOPONIMO, ' ')) as INDIRIZZO, COD_VIA as CODICEVIARIO from { qTable.ToString()} where toponimo is not null";

            return this._db.ExecuteReader(sql, mp =>
            {

            },
            reader => new DettagliVia
            {
        ...
        {
            var sql = new DistinctQueryBuilder(this._db);
            var paricelleCatastali = new ParicelleCatastaliTable(this._tablePrefix, this._nameProvider);
        ...
        internal Ravenna2Result GetCivico(string codVia, string civico)
        {
            ...
            var ra012 = new Ra012Table(this._tablePrefix, this._nameProvider);
            var ra147 = new Ra147Table(this._tablePrefix, this._nameProvider);
