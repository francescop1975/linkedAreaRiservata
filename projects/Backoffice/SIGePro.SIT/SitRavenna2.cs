...
using Init.SIGePro.Sit.Ravenna2.Oracle;
using Init.SIGePro.Sit.Ravenna2.SQLServer;
...
namespace Init.SIGePro.Sit
{
    public class SIT_RAVENNA2 : SitBaseV2
    {
        ...
        public override void SetupVerticalizzazione()
        {
            ...
            var db = new DataBase(v.ConnectionString, v.DatabaseClientType);

            var tableNameProvider = v.DatabaseClientType == ProviderType.OracleClient ? 
                   (IDatabaseTableNameProvider)new OracleTableNameProvider() : 
                   (IDatabaseTableNameProvider)new SQLServerTableNameProvider();

            this._dbClient = new Ravenna2DbClient(db, v.PrefissoTabelle, tableNameProvider);
