...
namespace Init.SIGePro.Verticalizzazioni
{
	...
	public class Verticalizzazione
	{
		...
		/// </summary>
		public Verticalizzazione(string alias , string nome, string software = "TT", string codiceComune = "")
		{
        ...
        private void Read()
        {
            var authInfo = AuthenticationManager.Login(_alias, Settings.Default.SigeproSecurityUsername, Settings.Default.SigeproSecurityPassword, ContextType.ExternalUsers);

            if (authInfo == null)
                throw new Exception(String.Format("AUTHENTICATION INFO NON VALORIZZATO, IdComuneAlias: {0}", _alias));
        ...
        public DateTime? GetDate(string paramName)
        {
            ...
            if (!Parametri.ContainsKey(paramName)) return null;
        ...
        {
            if (Parametri == null) return;
            Parametri[paramName] = (value.HasValue) ? value.Value.ToString("dd/MM/yyyy") : null;
        }
        ...
        {
            if (Parametri == null) return;
            Parametri[paramName] = (value.HasValue) ? value.ToString() : null;
        }
