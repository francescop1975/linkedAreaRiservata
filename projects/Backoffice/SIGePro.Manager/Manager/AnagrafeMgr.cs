...
namespace Init.SIGePro.Manager
{
	public class AnagrafeMgr : BaseManager//, IAnagrafeManager
	{
        ...
        public bool EscludiControlliSuAnagraficheDisabilitate { get; set; } = false;
        public bool ForzaInserimentoAnagrafe { get; set; } = false;
        public bool RicercaSoloCF_PIVA { get; set; } = false;
        public bool EscludiVerificaIncongruenze { get; set; } = false;
		...
		private void ChildInsert(Anagrafe p_class)
		{
			...
			foreach (AnagrafeDyn2ModelliT id2mt in p_class.AnagrafeDyn2ModelliT)
			{
				...
				var dap = new AnagrafeDyn2DataAccessFactory(db, id2mt.Idcomune, id2mt.Codiceanagrafe.Value);
				var loader = new ModelloDinamicoLoader(dap, p_class.IDCOMUNE, ModelloDinamicoLoader.TipoModelloDinamicoEnum.Backoffice);
				var mda = new ModelloDinamicoAnagrafica(loader, id2mt.FkD2mtId.Value, 0, false);
