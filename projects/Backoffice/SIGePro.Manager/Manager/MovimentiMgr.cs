...
namespace Init.SIGePro.Manager
{
	...
	public class MovimentiMgr : BaseManager
	{
		...
		public bool MovimentoEsistenteSuIstanza(string idComune, int codiceIstanza, string tipoMovimento)
		{
            if (string.IsNullOrWhiteSpace(idComune))
            {
                throw new ArgumentException($"'{nameof(idComune)}' non può essere Null o uno spazio vuoto", nameof(idComune));
            }

            if (string.IsNullOrWhiteSpace(tipoMovimento))
            {
                throw new ArgumentException($"'{nameof(tipoMovimento)}' non può essere Null o uno spazio vuoto", nameof(tipoMovimento));
            }

            var request = new Movimenti 
			{ 
				IDCOMUNE = idComune, 
			...
			request.OthersWhereClause.Add("DATA IS NOT NULL");

			

			return db.GetClassList(request).ToList<Movimenti>().Count() > 0;
		}
		...
		//		{
		//			istanzeOneriMgr.UpdatePrezzoIstruttoria(o.IDCOMUNE, o.ID, somma);
		...
		//			istanzeOneriMgr.UpdateDataPagamento(onere.IDCOMUNE, onere.ID, cls.DATA);
