...
{ 	
	public class IstanzeAllegatiMgr: BaseManager
	{
		...
		private void Validate(IstanzeAllegati p_class, Init.SIGePro.Validator.AmbitoValidazione ambitoValidazione)
		{
			...
			if (String.IsNullOrEmpty( p_class.VERIFICATO ) )
				p_class.VERIFICATO = "0";

			if (String.IsNullOrEmpty( p_class.CONTROLLOOK ) )
				p_class.CONTROLLOOK = "0";

			if (String.IsNullOrEmpty( p_class.PRESENTE ) )
				p_class.PRESENTE = "0";

			if (String.IsNullOrEmpty( p_class.SEENDO ) )
				p_class.SEENDO = "0";

			if (String.IsNullOrEmpty( p_class.SELEZIONATO ) )
