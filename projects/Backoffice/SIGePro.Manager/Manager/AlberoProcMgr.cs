namespace Init.SIGePro.Manager
{
    ...
    public partial class AlberoProcMgr
    {
        ...
        internal int GetCodiceInterventoProcDaCodiceIstanza(string idComune, int codiceIstanza) {
            var istanza = new IstanzeMgr(this.db).GetById(idComune, codiceIstanza);
            if( istanza == null )
            {
        ...
        public String GetPrimaOccorrenzaMetadato( string idComune, int idIntervento, string chiaveMetadato)
        {
            ...
            try
            {
                ...
                using (IDbCommand cmd = db.CreateCommand(sql))
                {
                    cmd.Parameters.Add(db.CreateParameter("idcomune", idComune));
                    cmd.Parameters.Add(db.CreateParameter("software", ramoAlbero.SOFTWARE));
                    cmd.Parameters.Add(db.CreateParameter("chiave", chiaveMetadato));
        ...
        {
            var sql = $"select SC_NOTE from alberoproc where idcomune = {db.Specifics.QueryParameterName("idcomune")} and SC_ID = {db.Specifics.QueryParameterName("scId")}";

            return db.ExecuteScalar(sql, "", mp =>
            {
