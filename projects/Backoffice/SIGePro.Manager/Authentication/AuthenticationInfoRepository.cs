...
namespace Init.SIGePro.Manager.Authentication
{
	...
	internal class AuthenticationInfoRepositoryFactory
	{
		...
		{
			private SigeproSecurityProxy _sigeproSecurityProxy = new SigeproSecurityProxy();
			...
			public AuthenticationInfo GetByToken(string token, TipiAmbiente tipiAmbiente)
			{
				...
				var checkResult = _sigeproSecurityProxy.CheckToken(chkReq);
				...
				var dbInfo = _sigeproSecurityProxy.GetDbConnectionInfo(dbInfoReq);
				...
				var dbInfoLocal = tipiAmbiente == TipiAmbiente.DOTNET ? dbInfo : _sigeproSecurityProxy.GetDbConnectionInfo(dbInfoReq);
			...
			public AuthenticationInfo GetByLoginInfo(string alias, string userId, string password, ContextType tipoContesto)
			{
				...
				var response = _sigeproSecurityProxy.Login(req);
