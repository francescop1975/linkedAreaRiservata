        ...
        private void EliminaValoriCampiInternal(DatiIdentificativiModello modello, IEnumerable<DatiIdentificativiCampo> campiDaEliminare)
        {
            foreach (var campo in campiDaEliminare)
            {
                var sql = $@"delete from mercatidyn2dati where 
                                        ...
                                        codicemercato={_database.Specifics.QueryParameterName("codicemercato")} and
                ...
                _database.ExecuteNonQuery(sql, mp =>
                {
                    ...
                    mp.AddParameter("codicemercato", _codiceMercato);
        ...
        public SerializableDictionary<int, IEnumerable<IValoreCampo>> GetValoriCampiDaIdModello(int idModello, int indiceModello)
        {
            var sql = $@"select 
						...
						WHERE
							...
							mercatiDyn2Dati.idcomune = {_database.Specifics.QueryParameterName("idcomune")} and									
							dyn2_modellid.fk_d2mt_id = {_database.Specifics.QueryParameterName("idModello")} and
							mercatiDyn2Dati.codicemercato = {_database.Specifics.QueryParameterName("codiceMercato")} and
							mercatiDyn2Dati.indice = {_database.Specifics.QueryParameterName("indiceModello")}
						order by 
            ...
            try
            {
                ...
                var sql = $@"INSERT INTO mercatidyn2dati (idcomune, codicemercato, fk_d2c_id, valore, indice, valoredecodificato, indice_molteplicita) VALUES 
                                (
                                    ...
                                    {_database.Specifics.QueryParameterName("codiceMercato")}, 
