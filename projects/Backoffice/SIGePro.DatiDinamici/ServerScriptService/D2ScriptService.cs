    ...
    public class D2ScriptService
	{
		...
		public StatoModello CampoModificato(string idCampo, int indice, string valore, string valoreDecodificato)
		{
			...
			lock (threadLock)
			{
				...
				try
				{
					...
					campo.ListaValori[indice].SetValore(valore, valoreDecodificato);
		...
		private StatoModello GetStatoModello()
		{
			return new StatoModello
			{
				...
				necessitaReloadInterfaccia = NecessitaReloadInterfaccia()
			};
		}
        ...
        private bool NecessitaReloadInterfaccia()
        {
			if (ModelloDinamicoRenderer.DataSourceAttuale == null)
				return false;

			return ModelloDinamicoRenderer.DataSourceAttuale.NecessitaReloadInterfaccia;
		}
		...
		private UidCampo[] GetStatoCampiNonVisibili()
		{
			...
			return ModelloDinamicoRenderer
					.DataSourceAttuale
					.GetIdCampiDaStatoVisibilita(StatoVisibilitaCampoEnum.NonVisibile)
					.Select(idCampo => new UidCampo
					{
						id = idCampo.Id,
						indice = idCampo.IndiceMolteplicita
		...
		private UidCampo[] GetStatoCampiVisibili()
		{
			...
			return ModelloDinamicoRenderer
					.DataSourceAttuale
					.GetIdCampiDaStatoVisibilita(StatoVisibilitaCampoEnum.Visibile)
					.Select(idCampo => new UidCampo
					{
						id = idCampo.Id,
						indice = idCampo.IndiceMolteplicita
