...
function ControlloUploadDatiDinamici($, campoDatiDinamici, options) {
    ...
    this.bottoneElimina = $(`<a href='#' class='${options.classeBottoneElimina}'><i class='fa fa-trash'></i></a>`)
                            .appendTo(this.uploadResultDiv);

    this.toHumanReadable = (len) => {
    ...
    if ((dimensioneMassima && dimensioneMassima !== "0") || estensioniAmmesse) {
        ...
        if (dimensioneMassima && dimensioneMassima !== "0") {
            $("<div><b>Dimensione massima:</b> " + this.toHumanReadable(parseInt(dimensioneMassima * 1024)) + " </div> ").appendTo(helpCtrl);
        }
    ...
    this.inviaFile = async (event) => {
        ...
        let uploadUrl = this.options.uploadHandler + '?firma=' + verificaFirmaDigitale + '&max=' + dimensioneMassima + "&es=" + estensioniAmmesse;
    ...
    this.uploadCompletato = function (parsedResult) {
        ...
        try {
            ...
            if (typeof (nascondiModalCaricamento) !== "undefined") {
    ...
    this.uploadIniziato = function () {
        if (typeof (mostraModalCaricamento) !== "undefined") {
    ...
    this.impostaDatiFile = (codiceOggetto, nomeFile, dimensione, mime)  => {
