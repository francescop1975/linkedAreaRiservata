...
namespace Init.SIGePro.DatiDinamici.ValidazioneValoriCampi
{
		...
		internal IEnumerable<ErroreValidazione> OnGetErroriDiValidazione()
		{
			for (int i = 0; i < this._campo.ListaValori.Count; i++)
			{
				...
				foreach (var ex in validatore.GetErroriDiValidazione())
				{
					...
					{
						if (!String.IsNullOrEmpty(this._indicatoreRigaErrore))
						{
							msg += " ";
	...
	internal class CampiMultipliObbligatoriValidator : ListaValoriValidatorBase
	{
		...
		public CampiMultipliObbligatoriValidator(CampoDinamicoBase campo)
			: base(campo, "Campo obbligatorio", "")
		{
		}
	...
	{
		public CheckboxesObbligatorieValidator(CampoDinamicoBase campo,
			// string messaggioErrore = "E'obbligatorio selezionare la casella \"{0}\"", 
			string messaggioErrore = "Campo obbligatorio",
			string indicatoreRiga = ""/*"alla riga/blocco {0}"*/)
	...
	{
		public RegexCampiValidator(CampoDinamicoBase campo, string messaggioErrore = "Il campo \"{0}\" contiene caratteri non validi", string indicatoreRiga = "(riga/blocco {0})")
			: base(campo, messaggioErrore, indicatoreRiga)
		{
		}
